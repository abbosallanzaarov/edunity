...
<div class="content-wrapper">
    ...
    <form action="{{ route('warning.store') }}" method="post" class="row">
        ...
        <div class="col-md-12">
            <h4 class="text-center">Siz 
                @if($warning->student_id)
                    {{ $warning->student_id }} talaba
                @else
                    {{ $warning->mentor_id }} mentor
                @endif
                ni ogohlantirishini tahrirlamoqdasiz!</h4>
            <hr>
        </div>
        <div class="col-md-6">
        ...
        <div class="col-md-12">
            <textarea name="warning" id="" cols="100" rows="10" class="w-100 form-control m-1">{{ $warning->warning }}</textarea>
        </div>

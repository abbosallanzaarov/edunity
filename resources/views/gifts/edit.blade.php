...
@section('content')
    <div class="row mb-5 mt-5">
        <form action="{{ route('gift.update', $gift->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('patch')
            <h1 class="text-center">{{ $gift->name }} ni Tahrirlang</h1>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input name="title" value="{{ $gift->title }}" type="text" required class="form-control">
            </div>
            <div class="mb-3">
                <label for="exampleFormControlReadOnlyInputPlain1" class="form-label">Qisqacha Mazmuni</label>
                <textarea name="desc" class="form-control">{{ $gift->desc }}</textarea>
            </div>
            <div class="mb-3">
                <label for="exampleFormControlReadOnlyInputPlain1" class="form-label">Image</label>
                <input type="file" {{ $gift->image }} name="image" class="form-control">
            </div>
            <div class="mb-3">
                ...
                <input type="number" name="coin" value="{{ $gift->coin }}" class="form-control">
            </div>
            <div class="mb-3">
